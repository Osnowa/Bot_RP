...
async def process_command_start(message: Message):
    with connect() as conn:
        ...
        user = users.get_user(telegram_id, conn)

        if not user:
            users.add_user(telegram_id, conn)
...
async def command_sbros(message: Message):
    with connect() as conn:
        telegram_id = message.from_user.id
        games.sbros_static(telegram_id, conn)
...
async def process_command_help(message: Message):
    with connect() as conn:
        user_data = users.get_user(message.from_user.id, conn)
...
async def callback_figure(callback: CallbackQuery):
    with connect() as conn:
        ...
        if result == "draw":
            ...
            games.add_draw(telegram_id, conn)
            ...
            await callback.message.edit_text(f"Я выбросил {otv_fig}\n"
                                             ...
                                             f"Ты проиграл!",
                                             reply_markup=callback.message.reply_markup)
            games.add_los(telegram_id, conn)
            ...
            await callback.message.edit_text(f"Я выбросил {otv_fig}\n"
                                             ...
                                             reply_markup=callback.message.reply_markup)
            games.add_wins(telegram_id, conn)
